...
void Testbench::do_modules() {
  ...
  for (int y = -pad; y < (int)height + pad; ++y) {
    ...
    initiator.write_to_socket(MOD_MM_BASE + MOD_INPUT_ADDR, mask.data(),
                              row_data.data(), padded_width);

    // Read results using burst
    initiator.read_from_socket(MOD_MM_BASE + MOD_RESULT_ADDR, mask.data(),
                               row_result.data(), padded_width);
