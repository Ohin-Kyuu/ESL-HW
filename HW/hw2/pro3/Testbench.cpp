...
void Testbench::do_modules() {
  ...
  for (int y = -pad; y < (int)height + pad; ++y) {
    for (int x = -pad; x != (int)width + pad; ++x) {
      ...
      initiator.write_to_socket(MOD_MM_BASE + MOD_INPUT_ADDR, mask, &pixel, 1);

      // Read results
      initiator.read_from_socket(MOD_MM_BASE + MOD_RESULT_ADDR, mask, &result,
                                 1);
      int cur_x = x - pad;
      int cur_y = y - pad;
      if (cur_x >= 0 && cur_x < (int)width && cur_y >= 0 &&
          cur_y < (int)height) {
        target_bitmap[cur_y * width + cur_x] = result;
      }
    }
  }
