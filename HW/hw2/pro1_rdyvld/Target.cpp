...
void Target::rdyvld_thread() {
  while (true) {
    rdy.write(true);
    while (vld.read() == false) {
    ...
    // Simulation Delay for filter logic
    wait(2 * CLOCK_PERIOD, SC_NS);
  }
}
...
void Target::blocking_transport(tlm::tlm_generic_payload &payload,
  ...
  case tlm::TLM_READ_COMMAND:
    ...
    delay += sc_time(payload.get_data_length() * 2 * CLOCK_PERIOD, SC_NS);
    break;
