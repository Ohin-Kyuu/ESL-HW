    ...
    for (x = -1; x <= (int)width; ++x) {
      ...
      for (int dy = -2; dy <= 0; ++dy) {
        for (int dx = -1; dx <= 1; ++dx) {
          int cur_x = x + dx;
          int cur_y = y + dy;
          sc_uint<8> gray = (cur_x >= 0 && cur_x < (int)width && cur_y >= 0 &&
                             cur_y < (int)height)
                                ? gray_bitmap[cur_y * width + cur_x]
                                : 0;
          o_g.write(gray);
          wait();
        }
      ...
      // Use Padding to reset buffer in Scharr (Flush)
      // when the row(y) is not the start row, need to flush buffer
      if (y > 0 || x > 0) {
        if (i_result.num_available() == 0)
          wait(i_result.data_written_event());
        total = i_result.read();

        // target only available when x > 0 (x=1 {-1, 0 ,1})
        if (x > 0) {
          target_bitmap[y * width + (x - 1)] = total;
        }
      }
    }
  }
