...
void Testbench::do_modules() {
  ...
  for (y = 0; y < (int)height; ++y) {
    for (x = 0; x < (int)width; ++x) {
      ...
      // Scharr Window
      for (v_s = -yBound; v_s != yBound + adjustY; ++v_s) {
        for (u_s = -xBound; u_s != xBound + adjustX; ++u_s) {

          // Median Window
          for (v_m = -yBound; v_m != yBound + adjustY; ++v_m) {
            for (u_m = -xBound; u_m != xBound + adjustX; ++u_m) {
              sc_uint<8> gray = 0;
              ...
              if (cur_x >= 0 && cur_x < (int)width && cur_y >= 0 &&
                  cur_y < (int)height) {
                gray = gray_bitmap[cur_y * width + cur_x];
              } else {
                gray = 0;
              }

              o_g.write(gray);
              wait(); // emulate channel delay
            }
          }
        }
      }
