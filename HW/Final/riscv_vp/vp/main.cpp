...
public:
  ...
  addr_t mem_size =
      1024 * 1024 *
      32; // 32 MB ram, to place it before the CLINT and run the base examples
          // (assume memory start at zero) without modifications
  ...
  // Add
  addr_t HammingMatch_start_addr = 0x73000000;
  addr_t HammingMatch_size = 0x01000000;
  addr_t HammingMatch_end_addr =
      HammingMatch_start_addr + HammingMatch_size - 1;

  addr_t HammingMatch2_start_addr = 0x74000000;
  addr_t HammingMatch2_end_addr =
      HammingMatch2_start_addr + HammingMatch_size - 1;

  addr_t HammingMatch3_start_addr = 0x75000000;
  addr_t HammingMatch3_end_addr =
      HammingMatch3_start_addr + HammingMatch_size - 1;

  addr_t HammingMatch4_start_addr = 0x76000000;
  addr_t HammingMatch4_end_addr =
      HammingMatch4_start_addr + HammingMatch_size - 1;
...
int sc_main(int argc, char **argv) {
  ...
  // Add
  HammingMatch hm_pe0("hm_pe0", 6);
  HammingMatch hm_pe1("hm_pe1", 8);
  HammingMatch hm_pe2("hm_pe2", 9);
  HammingMatch hm_pe3("hm_pe3", 10);

  MemoryDMI dmi = MemoryDMI::create_start_size_mapping(
      mem.data, opt.mem_start_addr, mem.size);
   ...
   * mainly used together with the syscall handler, this helps for certain
   * floats.
   * https://github.com/riscv-non-isa/riscv-elf-psabi-doc/blob/master/riscv-elf.adoc
   */
  core0.init(instr_mem_if0, data_mem_if0, &clint, entry_point,
             (opt.mem_end_addr - 3));
  core1.init(instr_mem_if1, data_mem_if1, &clint, entry_point,
             (opt.mem_end_addr - 32767));
  core2.init(instr_mem_if2, data_mem_if2, &clint, entry_point,
             (opt.mem_end_addr - 65535));
  core3.init(instr_mem_if3, data_mem_if3, &clint, entry_point,
             (opt.mem_end_addr - 98303));
  ...
  // address mapping
  {
    ...
    bus.ports[it++] =
        new PortMapping(opt.sensor_start_addr, opt.sensor_end_addr);
    ...
    // Add
    bus.ports[it++] =
        new PortMapping(opt.HammingMatch_start_addr, opt.HammingMatch_end_addr);
    bus.ports[it++] = new PortMapping(opt.HammingMatch2_start_addr,
                                      opt.HammingMatch2_end_addr);
    bus.ports[it++] = new PortMapping(opt.HammingMatch3_start_addr,
                                      opt.HammingMatch3_end_addr);
    bus.ports[it++] = new PortMapping(opt.HammingMatch4_start_addr,
                                      opt.HammingMatch4_end_addr);
  }
  ...
  PeripheralWriteConnector dma_connector(
      "SimpleDMA-Connector"); // to respect ISS bus locking
...

  {
    ...
    // Add
    bus.isocks[it++].bind(hm_pe0.tsock);
    bus.isocks[it++].bind(hm_pe1.tsock);
    bus.isocks[it++].bind(hm_pe2.tsock);
    bus.isocks[it++].bind(hm_pe3.tsock);
  }
  ...
  if (opt.use_debug_runner) {
    auto server0 =
        new GDBServer("GDBServer0", threads, &dbg_if, opt.debug_port);
    auto server1 =
        new GDBServer("GDBServer1", threads, &dbg_if, opt.debug_port);
    auto server2 =
        new GDBServer("GDBServer2", threads, &dbg_if, opt.debug_port);
    auto server3 =
        new GDBServer("GDBServer3", threads, &dbg_if, opt.debug_port);
  ...
  double dma_transfer_cycles =
      dma.transfer_time / sc_core::sc_time(10, sc_core::SC_NS);

  std::cout << "\n========================================================\n";
  std::cout << "                 HW Module Latency Report                 \n";
  std::cout << "========================================================\n";
  std::cout << "PE0 Compute Time   : " << hm_pe0.total_compute_cycles
            << " cycles\n";
  std::cout << "PE1 Compute Time   : " << hm_pe1.total_compute_cycles
            << " cycles\n";
  std::cout << "PE2 Compute Time   : " << hm_pe2.total_compute_cycles
            << " cycles\n";
  std::cout << "PE3 Compute Time   : " << hm_pe3.total_compute_cycles
            << " cycles\n";
  std::cout << "--------------------------------------------------------\n";
  std::cout << "Total DMA Transfer : " << dma_transfer_cycles << " cycles\n";
  std::cout << "========================================================\n";
  return 0;
}
