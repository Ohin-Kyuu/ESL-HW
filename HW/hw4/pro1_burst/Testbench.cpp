...
void Testbench::do_modules() {
  ...
  int pad = 2;
  int padded_width = (int)width + 4;
  std::vector<unsigned char> mask(padded_width, 0xff);

  // Row Data for Burst
  std::vector<unsigned char> row_data(padded_width);
  std::vector<unsigned char> row_result(padded_width);

  for (int y = -pad; y < (int)height + pad; ++y) {

    // Write pixels to socket using burst
    for (int x = -pad; x < (int)width + pad; ++x) {
      ...
      row_data[x + pad] = pixel;
    }
    initiator.write_to_socket(MOD_INPUT_ADDR, mask.data(), row_data.data(),
                              padded_width);

    // Read results using burst
    initiator.read_from_socket(MOD_RESULT_ADDR, mask.data(), row_result.data(),
                               padded_width);
    // No fifo, so we need to skip the pad
    //   for (int x = 0; x < padded_width; ++x) {
    //     int cur_x = x - 2 * pad; // result start from x = 4
    //     int cur_y = y - pad;
    //
    //     if (cur_x >= 0 && cur_x < (int)width && cur_y >= 0 &&
    //         cur_y < (int)height) {
    //       target_bitmap[cur_y * width + cur_x] = row_result[x];
    //     }
    //   }
    // }

    // Using fifo, so just read the result
    int cur_y = y - pad;
    if (cur_y >= 0 && cur_y < (int)height) {
      for (int x = 0; x < (int)width; ++x) {
        target_bitmap[cur_y * width + x] = row_result[x];
      }
    }
  }
  sc_stop();
