...
public:
  ...
  // Add
  addr_t target_start_addr = 0x73000000;
  addr_t target_size = 0x01000000;
  addr_t target_end_addr = target_start_addr + target_size - 1;
...
int sc_main(int argc, char **argv) {
   ...
   */
  core.init(instr_mem_if, data_mem_if, &clint, entry_point,
  ...
  // address mapping
  {
    ...
    // Add
    bus.ports[it++] =
        new PortMapping(opt.target_start_addr, opt.target_end_addr);
  }
...

  {
    ...
    // Add
    bus.isocks[it++].bind(target.tsock);
  }
  ...
  double transfer_cycles =
      dma.transfer_time / sc_core::sc_time(CLOCK_PERIOD, SC_NS);
  std::cout << "========================================\n";
  std::cout << "            HW module latency           \n";
  std::cout << "========================================\n";
  std::cout << "Filter (median+scharr) : " << target.filter_cycles
            << " cycles\n";
  std::cout << "Result readout         : " << target.readout_cycles
            << " cycles\n";
  double dma_transfer =
      transfer_cycles - target.filter_cycles - target.readout_cycles;
  std::cout << "DMA Transfer Time      : " << dma_transfer << " cycles\n";
